...
_os.observe(self, "currentIndexChanged", function (index, widget) {
   ...
   if (_object) {
      ...
      dmz.object.text(_object, dmz.saeConst.OSAttr, _os.itemText(index));
      ...
      if (value === 1) {
         ...
         if (type && _object) {
            ...
            if (handle) {
               ...
               dmz.object.link(dmz.saeConst.ServiceLink, _object, handle);
   ...
   if (Mode === dmz.module.Activate) {
      ...
      module.addInspector(self, _form, ComputerType, function (handle) {
         ...
         var name = dmz.object.text(handle, dmz.saeConst.NameAttr)
           , os = dmz.object.text(handle, dmz.saeConst.OSAttr)
           , type = dmz.object.type(handle)
           , services = dmz.object.subLinks(handle, dmz.saeConst.ServiceLink);
           ;
...
dmz.module.subscribe(self, "objectInit", function (Mode, module) {
   ...
   if (Mode === dmz.module.Activate) {
      ...
      module.addInit(self, ComputerType, function (handle, type) {
         ...
         if (!dmz.object.text(handle, dmz.saeConst.OSAttr)) {

            if (os) { dmz.object.text(handle, dmz.saeConst.OSAttr, os); }
         }
      }); 
   }
});
...
dmz.object.text.observe(self, dmz.saeConst.OSAttr, function (handle, attr, value) {
